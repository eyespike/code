...
bool floodIconUpdated = false;
bool floodConfirmedIconUpdated = false;
bool waterShutoffIconUpdated = false;
bool noticeSentIconUpdated = false;
...
void convert_array_to_image_data ()
{
	...
	if(diff > 0)
	{
		...
		//MagickExportImagePixels(m_wand,0,0,80,60, "RGB", CharPixel, videoFrameBlock);
...
void* iterate_lepton(void *arg)
{
	...
	while(_active){
		...
		if(_monitorActive){
			...
			if(_currentIteration/30 > PUMP_START_DELAY_SECS){
